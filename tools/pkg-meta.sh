...
#
# Sourced, not run. $0 is still the build script, so SRC resolves relative to
# it and both builders can live at the top of the tree.

PKG=luci-app-fw-live
# The top of the tree. $0 is the build script when this is sourced by one, but
# the tests source it too and are not at the top, so they set this instead.
...
# cp -a would carry the checkout's own modes into the package, so the result
# depended on the builder's umask, and a checkout that came through a transport
# which drops the exec bit produced a package that could not run at all.
stage_files() {
	mkdir -p "$1"
	cp -a "$SRC/files/." "$1/"
