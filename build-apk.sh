...
# holding this script.
#
#   ./build-apk.sh  ->  luci-app-fw-live-<version>-r<release>.apk
#
...
# The ./ has to go, not just the dot: package-pack.mk uses find -printf "/%P",
# which strips the starting point, and busybox find has no -printf.
( cd "$ROOT" && find . \( -type f -o -type l \) | sed 's|^\./|/|' | sort ) > "$WORK/list"
mv "$WORK/list" "$META/$PKG.list"

# Anything that walks this list compares it against real paths, so a leading
# "//" is a silent mismatch rather than an error, and nothing else here looks
# inside the built package. It has been wrong once.
if grep -q '^//' "$META/$PKG.list"; then
	echo "build-apk.sh: the file list has doubled slashes, see the sed above" >&2
	exit 1
fi
